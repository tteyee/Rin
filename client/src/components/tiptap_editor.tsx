...
export function TiptapEditor({
  ...
  const [rawText, setRawText] = useState("");   // 마크다운 / HTML 직접 편집용
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      if (mode === "rich") {
        // 마크다운으로 직렬화해서 상위에 전달
        setContent((editor.storage.markdown as any).getMarkdown());
      }
    },
  });
  ...
  useEffect(() => {
    ...
    // content 가 마크다운으로 들어온다고 가정
    const current = (editor.storage.markdown as any).getMarkdown();
    if (current !== content) {
  ...
  const switchMode = (next: EditorMode) => {
    ...
    if (mode === "rich") {
      // rich → 다른 모드: 에디터 내용 → rawText
      const md = (editor.storage.markdown as any).getMarkdown();
      if (next === "markdown") setRawText(md);
      if (next === "html") setRawText(editor.getHTML());
    } else if (next === "rich") {
      // 다른 모드 → rich: rawText → 에디터 반영
      if (mode === "markdown") {
      ...
      // markdown ↔ html 전환
      if (mode === "markdown" && next === "html") {
        editor.commands.setContent(rawText);
        setRawText(editor.getHTML());
      } else if (mode === "html" && next === "markdown") {
        editor.commands.setContent(rawText);
        setRawText((editor.storage.markdown as any).getMarkdown());
      }
    }
    setMode(next);
  };

  // rawText 변경 시 상위 state 동기화
  useEffect(() => {
    ...
    <div className="flex flex-col gap-0 sm:gap-3">
      ...
      >
        ...
        {mode === "rich" && (
          <EditorContent
            editor={editor}
            className="tiptap-editor prose prose-neutral dark:prose-invert max-w-none px-4 py-4 focus:outline-none"
            style={{ minHeight: height }}
          />
        )}
